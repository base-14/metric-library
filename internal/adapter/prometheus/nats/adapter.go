...
const repoURL = "https://github.com/nats-io/prometheus-nats-exporter"
...
func (a *Adapter) Extract(_ context.Context, result *adapter.FetchResult) ([]*adapter.RawMetric, error) {
	...
	for _, entry := range entries {
		...
		prefix := systemPrefix(entry.Name())
		...
		for _, def := range defs {
			...
			// The "system" parameter in BuildFQName is a function arg,
			// not a file-level constant, so the parser can't resolve it.
			// Add the correct system prefix based on the source file.
			name := def.Name
...
// systemPrefix returns the Prometheus namespace prefix for a collector file.
// jsz.go uses JetStreamSystem ("jetstream"), everything else uses CoreSystem ("gnatsd").
func systemPrefix(filename string) string {
	if filename == "jsz.go" {
		return "jetstream"
	}
	return "gnatsd"
